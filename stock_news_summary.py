...
# Load API keys and token from environment variables
NEWSDATA_API_KEY = os.getenv("NEWSDATA_API_KEY")
OPENROUTER_API_KEY = os.getenv("OPENROUTER_API_KEY")
GH_TOKEN = os.getenv("GH_TOKEN")  # GitHub PAT (store as secret, NOT in code)

OUTPUT_FILE = "docs/latest_news.html"
REPO = "krishxd3/news-auto"  # Change if your repo name is different
BRANCH = "main"

os.makedirs("docs", exist_ok=True)

def fetch_news():
    print("📡 Fetching Indian stock news...")
    url = f"https://newsdata.io/api/1/latest?apikey={NEWSDATA_API_KEY}&q=Stock Market&country=in&language=en&timezone=Asia/Kolkata"
    response = requests.get(url)
    if response.status_code == 200:
        return response.json().get("results", [])
...
def summarize_with_deepseek(text):
    prompt = (
        f"Summarize this Indian stock market news in 2-3 lines "
        f"and describe its likely impact on Indian stock market or specific stocks:\n\n{text}"
    )
    headers = {
        "Authorization": f"Bearer {OPENROUTER_API_KEY}",
        "Content-Type": "application/json"
...
def git_commit_push():
    print("📤 Committing and pushing HTML to GitHub...")
